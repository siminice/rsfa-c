...
int DeleteResult(int h, a, b) {
}

int AddResult() {
  int more = 0;
  int rd, a, b, x, y, z, i, oldr;
  char rep[5];
  ...
  if (tbr<10) {
    ...
    else if (rep[0] < '0' || rep[0] > '2') return 1;
  }
  printf("Score: "); scanf("%d", &z);
  if (tbr<10 || res[a][b] < 0) {
    if (rep[0]!='k') rnd[a][b] = _rnd;
    res[a][b] = z;
  }
  else if (res2[a][b] < 0) {
    rnd2[a][b] = _rnd;
  ...
  if (res[a][b]>=0)
    printf("%s - %s  [%d-%d]  (round %2d, %s %2d)\n", 
      NameOf(L,id[a],year), NameOf(L,id[b],year),
      res[a][b]/100, res[a][b]%100, rnd[a][b]/1000, month[(rnd[a][b]%1000)/50-1], rnd[a][b]%50);
  if (res[b][a]>=0)
    printf("%s - %s  [%d-%d]  (round %2d, %s %2d)\n", 
      NameOf(L,id[b],year), NameOf(L,id[a],year),
      res[b][a]/100, res[b][a]%100, rnd[b][a]/1000, month[(rnd[b][a]%1000)/50-1], rnd[b][a]%50);
  if (res2[a][b]>=0)
    printf("%s - %s  [%d-%d]  (round %2d, %s %2d)\n", 
      NameOf(L,id[a],year), NameOf(L,id[b],year),
      res2[a][b]/100, res2[a][b]%100, rnd2[a][b]/1000, month[(rnd2[a][b]%1000)/50-1], rnd2[a][b]%50);
  if (res2[b][a]>=0)
    printf("%s - %s  [%d-%d]  (round %2d, %s %2d)\n", 
      NameOf(L,id[b],year), NameOf(L,id[a],year),
      res2[b][a]/100, res2[b][a]%100, rnd2[b][a]/1000, month[(rnd2[b][a]%1000)/50-1], rnd2[b][a]%50);
  gla = a; glb = b;
  return 1;
}
