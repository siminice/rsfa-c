...
//  printf("Score: "); scanf("%d", &z);
  ...
  if (tbr<10 || res[a][b] < 0) {
    if (rep[0]!='k') rnd[a][b] = 1000*(_rnd/1000)+z;
    res[a][b] = 100*x+y;
  }
  else if (res2[a][b] < 0) {
...
int Extract(char *filename) {
  ...
  char *tk, *tkw, *tkm, *tkd, *tkz, *tkj, *tkyr, *tks, *t1, *t2, *tkx, *tky;
...
//      tkyr = strtok(NULL, ",<\n");
      ...
      if (m1>0) m = m1;
      if (m1>0 && tkz && strlen(tkz)>0) z1  = atoi(tkz); else z1 = 0;
      if (z1>0) z = z1;
      if (strstr(s, "full-date")!=NULL) {
        tkz = strtok(tk, "-");
        tkm = strtok(NULL, "-");
        z1 = atoi(tkz);
        if (tkm) m1 = atoi(tkm);
        if (m1>0 && z1>0) { z = z1; m = m1; }
      }
