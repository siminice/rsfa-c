...
int main(int argc, char* argv[]) {
  ...
  do {
    ...
    if (f!=NULL) {
      ...
      printf("New filename:"); scanf("%s", s);
    }
  } while(f!=NULL);
  int k=0; while (k<strlen(s) && s[k]!='.') k++;
  int y = atoi(s+k+1);
  printf("No.of teams:"); scanf("%d", &n);
  int *id = new int[n];
  printf("PPV:"); scanf("%d", &ppv);
  printf("Tie-break rule:"); scanf("%d", &tbr);
  printf("Promoted:"); scanf("%d", &pr1);
  printf("Promotion play-off:"); scanf("%d", &pr2);
  printf("Relegated:"); scanf("%d", &rel1);
  printf("Relegation play-off:"); scanf("%d", &rel2);
  for (i=0; i<n; i++) {
