...
int Extract(char *filename) {
  ...
  if (NULL == f) { printf("File %s not found.\n", filename); return 0; }
  // Loading file
  char *s, *t, *w, *tkr[100], *tkd[100];
  int x, y, i, j, k, q, z, m, r, found;
  int rn[64];
  s = new char[1024];
  t = new char[1024];
  w = new char[1024];
  for (i=0; i<n; i++) {
    fgets(s,1024,f);
    tkr[0] = strtok(s, "\t\n");
    for (j=1; j<100; j++) tkr[j] = strtok(NULL, "\t\n");
    fgets(t,1024,f);
    strcpy(w, t);
    tkd[0] = strtok(t, "\t\n");
    for (j=1; j<100; j++) tkd[j] = strtok(NULL, "\t\n");

    /* get scores */
    k=1;
    found=0;
    while (found==0 && tkr[k]!=NULL) {
      if (isNumeric2(tkr[k])>0) found = 1;
      else k++;
    }
    if (found>0) {
      q = k;
    ...
    while (j<n) {
      if (w[q]=='\t' || w[q]=='\n') { 
        ...
        rn[j] = 1;
        while (q<strlen(w) && w[q]!='\t' && w[q]!='\n') q++;
        if (w[q]=='\t') q++;
        if (w[q]!='\t') { // modified date
          while (w[q]!='\t' && w[q]!='\n') q++;
          if (w[q]=='\t') q++;
          j++;
        }
      ...
      for (j=0; j<n; j++) {
        if (i!=j) {
          if (rn[j]>0) {          
            ...
            round[i][j] = 1000*r+z;
            int rr = r + n-1+n%2;
            if (r<=n-1+n%2 && round[j][i]<0) round[j][i] = 1000*rr+ziua[rr];
